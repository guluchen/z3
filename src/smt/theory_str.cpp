    ...
    model_value_proc *theory_str::mk_value(enode *const n, model_generator& mg) {
        ...
        if (get_len_value(n->get_owner(), vLen)) {
            ...
            for (int i = 0; i < leafNodes.size(); ++i) {
                expr *value = queryTheoryArithCore(mk_strlen(leafNodes[i]), mg);
                if (value != nullptr) {
                    rational tmp;
                    if (m_autil.is_numeral(value, tmp))
                        vLen = vLen + tmp;
                    STRACE("str", tout << __LINE__ << " len value :  " << mk_pp(mk_strlen(leafNodes[i]), m) << ": " << mk_pp(value, m) << " --> " << vLen
                                       << std::endl;);
                }
                else {
                    vLen = -1;
        ...
        if (val != nullptr) {
            ...
            if (isImportant(owner.get()) && !u.str.is_concat(owner.get())) {
                STRACE("str", tout << __LINE__ << "mk_value for: " << mk_ismt2_pp(owner, m) << " (sort "
                                   << mk_ismt2_pp(m.get_sort(owner), m) << ")" << std::endl;);

//                if (!ctx.e_internalized(arrMap[owner.get()]))
//                    ctx.internalize(arrMap[owner.get()], false);

                enode* arrNode = ctx.get_enode(arrMap[owner.get()]);

                result = alloc(string_value_proc, *this, s, n->get_owner(), true, arrNode, vLen.get_int32());
                ...
                for (int i = 0; i < eqSet.size(); ++i) {
                    STRACE("str", tout << __LINE__ << "eq element: " << mk_pp(eqSet[i].get(), m) << " (sort "
                                       << mk_ismt2_pp(m.get_sort(eqSet[i].get()), m) << ")" << std::endl;);
                    if (isImportant(eqSet[i].get()) && !u.str.is_concat(eqSet[i].get())) {
                        STRACE("str", tout << __LINE__ << "eq element: " << mk_pp(arrMap[eqSet[i].get()], m) << " (sort "
                                           << mk_ismt2_pp(m.get_sort(arrMap[eqSet[i].get()]), m) << ")" << std::endl;);

//                        if (!ctx.e_internalized(arrMap[eqSet[i].get()]))
//                            ctx.internalize(arrMap[eqSet[i].get()], false);

                        enode* arrNode = ctx.get_enode(arrMap[eqSet[i].get()]);
                        STRACE("str", tout << __LINE__ << "eq element: " << mk_pp(arrMap[eqSet[i].get()], m) << " (sort "
                                           << mk_ismt2_pp(m.get_sort(arrMap[eqSet[i].get()]), m) << ")" << std::endl;);
                        result = alloc(string_value_proc, *this, s, n->get_owner(), true,
                                       arrNode, vLen.get_int32());
                        STRACE("str", tout << __LINE__ << "eq element: " << mk_pp(eqSet[i].get(), m) << " (sort "
                                           << mk_ismt2_pp(m.get_sort(eqSet[i].get()), m) << ")" << std::endl;);
                        found = true;
                ...
                if (!found) {
                    result = alloc(string_value_proc, *this, s, n->get_owner(), false, vLen.get_int32());
                }
            }
            ...
            if (importantNode != nullptr) {
                // add array
                result->add_entry(ctx.get_enode(arrMap[importantNode]));
    ...
    bool theory_str::isImportant(expr* n){
    ...
    bool theory_str::isImportant(expr* n, int &val){
        for (const auto& nn : uState.importantVars)
            if (nn.first == n) {
                val = nn.second;
                if (val < 0)
    ...
    std::set<expr*> theory_str::getDependency(expr* n){
        ...
        if (uState.eq_combination.find(n) != uState.eq_combination.end()) {
            for (const auto& nn : uState.eq_combination[n]) {
                if (u.str.is_concat(nn)) {
                    ptr_vector<expr> argVec;
                    get_nodes_in_concat(nn, argVec);
...
//                           << mk_ismt2_pp(n2, m) << " --> " << mk_pp(finalAxiom.get(), m) << std::endl;);
    ...
    void theory_str::push_scope_eh() {
        STRACE("str", tout << __FUNCTION__ << ": at level (" << m_scope_level << "/" << mful_scope_level << ")" << std::endl;);
        m_scope_level += 1;
        mful_scope_level += 1;
    ...
    void theory_str::pop_scope_eh(const unsigned num_scopes) {
        STRACE("str", tout << __FUNCTION__ << ": at level (" << m_scope_level << "/" << mful_scope_level << "/" << uState.level << ") pop " << num_scopes << std::endl;);

        if (mful_scope_level >= uState.level && mful_scope_level - (int)num_scopes < uState.level) {
            STRACE("str", tout << __FUNCTION__ << " " << mful_scope_level - (int)num_scopes << " vs " << uState.level << std::endl;);
            uState.reset();
        }

        m_scope_level -= num_scopes;
        int meaningful_scope = std::min(mful_scope_level, (int)num_scopes);
        mful_scope_level = mful_scope_level - meaningful_scope;
    ...
    final_check_status theory_str::final_check_eh() {
        ...
        context& ctx = get_context();
        (void) ctx;
        TRACE("str", tout << __FUNCTION__ << ": at level " << ctx.get_scope_level() << " vs " << uState.z3_level << std::endl;);
 ...
 */
    ...
    bool theory_str::underapproximation(
            ...
            std::set<std::pair<expr*, int>> importantVars,
            str::state root) {
        STRACE("str", tout << __LINE__ <<  " *** " << __FUNCTION__ << " *** (" << m_scope_level << "/" << mful_scope_level << ")" << connectingSize << std::endl;);
        ast_manager & m = get_manager();
        ...
        UnderApproxState state(m_scope_level, mful_scope_level, eq_combination, importantVars);
        if (state == uState)
            return false;
        else {
            uState = state;
    ...
    bool theory_str::underapproximation_repeat(){
        ...
        for (const auto& e : uState.assertingConstraints){
            ...
            if (!ctx.e_internalized(e))
                ctx.internalize(e, false);
            STRACE("str", tout << __LINE__ <<  " " << mk_pp(e, m) << ")\n";);
            assert_axiom(e);
        }
        return true;
    }
    ...
    void theory_str::handle_NOTEqual(){
        for (const auto &wi : m_wi_expr_memo) {
            if (!u.str.is_empty(wi.second.get()) && !u.str.is_empty(wi.first.get())) {
                STRACE("str", tout << __LINE__ <<  " not (" << wi.first << " = " << wi.second << ")\n";);
                expr* lhs = wi.first.get();
                expr* rhs = wi.second.get();
                expr* contain = nullptr;
                if (!isContains(lhs, contain) && !isContains(rhs, contain)) {
                    STRACE("str", tout << __LINE__ <<  " not (" << wi.first << " = " << wi.second << ")\n";);
                    zstring value;
    ...
    void theory_str::handle_NOTEqual_var(expr* lhs, expr* rhs){
        ...
        int len1, len2;
        if (isImportant(lhs, len1) && isImportant(rhs, len2)) {
            expr* arrLhs = arrMap[lhs];
            expr* arrRhs = arrMap[rhs];
        ...
        expr_ref axiom(m.mk_or(mk_not(m, notcause), createOrOperator(cases)), m);
        assert_axiom(axiom);
    ...
    void theory_str::handle_NOTEqual_const(expr* lhs, zstring rhs){
        ...
        if (isImportant(lhs)) {
            ...
            for (int i = 0; i < rhs.length(); ++i) {
                expr_ref_vector subcases(m);
                subcases.push_back(createGreaterEqOperator(lenLhs.get(), m_autil.mk_int(i + 1)));
                STRACE("str", tout << __LINE__ <<  "  " << mk_pp(createGreaterEqOperator(lenLhs.get(), m_autil.mk_int(i + 1)), m)  << ")\n";);
    ...
    void theory_str::handle_NOTContain(){
        for (const auto &wi : m_wi_expr_memo) {
            if (!u.str.is_empty(wi.second.get()) && !u.str.is_empty(wi.first.get())) {
                ...
                else if (isContains(rhs, contain)) {
                    zstring value;
                    if (u.str.is_string(contain, value))
...
//            assert_implication(mk_not(m, mk_contains(lhs, u.str.mk_string(rhs))), createAndOperator(cases));
//            assert_axiom(createAndOperator(cases));
    ...
    bool theory_str::isContains(expr* n, expr* &contain){
        if (u.str.is_concat(n)){
    ...
    void theory_str::initUnderapprox_repeat(){
        STRACE("str", tout << __LINE__ <<  " *** " << __FUNCTION__ << " *** " << connectingSize << std::endl;);
        ast_manager & m = get_manager();
        context & ctx = get_context();
        std::set<expr*> allStrExprs;
    ...
    bool theory_str::convertEqualities(std::map<expr*, std::vector<expr*>> eq_combination,
        ...
        std::vector<expr*> assertedConstraints;
        bool axiomAdded = false;
                ...
                for (const auto& element: it->second) {
                    ...
                    if (result != nullptr) {
                        ...
                        if (causes.find(it->first) != causes.end()) {
                            expr *tmp = createEqualOperator(causes[it->first], result);
                ...
                for (unsigned i = 0; i < it->second.size(); ++i)
                    for (unsigned j = i + 1; j < it->second.size(); ++j) {
                        ...
                        if (lhs.size() == 0 || rhs.size() == 0){
                            expr* tmp = constraintsIfEmpty(lhs, rhs);
                            assertedConstraints.push_back(tmp);
                            assert_axiom(tmp);
                            continue;
                        }
...
//		__debugPrint(logFile, "%d *** %s ***: %d: %d -> %d\n", __LINE__, __FUNCTION__, i, startPos, j);
    ...
    void theory_str::createInternalVar(expr* v){
        ...
        for (int i = start; i < end; ++i) {
            ...
            expr* lenConstraint = createGreaterEqOperator(v1, m_autil.mk_int(0));
            assert_axiom(lenConstraint);
...
//                assert_axiom(createGreaterEqOperator(v2, m_autil.mk_int(0)));
                expr* iteConstraint = createEqualOperator(v2, m_autil.mk_int(1));
                assert_axiom(iteConstraint);
...
//            adds.push_back(createMultiplyOperator(v1, v2));
        ...
        zstring val;
        if (u.str.is_string(v, val)){
            expr* sumConstraint = createEqualOperator(createAddOperator(adds), mk_int(val.length()));
            assert_axiom(sumConstraint);
            uState.addAssertingConstraints(sumConstraint);
        }
        else {
            expr* sumConstraint = createEqualOperator(createAddOperator(adds), u.str.mk_length(v));
            assert_axiom(sumConstraint);
            uState.addAssertingConstraints(sumConstraint);
        }
    }
...
//                });
    ...
    expr* theory_str::queryTheoryArithCore(expr* n, model_generator& mg){
        context& ctx = get_context();
        family_id afid = m_autil.get_family_id();
    ...
    expr* theory_str::queryTheoryArray(expr* n, model_generator& mg){
        ast_manager& m = get_manager();
        context& ctx = get_context();
    ...
    void theory_str::finalize_model(model_generator& mg) {
        ...
        for (const auto& n : variable_set){
            ...
            if (get_len_value(n, vLen)) {
                ...
                expr_ref len(m);
                len = mk_strlen(n);
                expr* value = queryTheoryArithCore(len, mg);
                if (value != nullptr) {
